...
type userServicer interface {
	...
	GetPersentOfUsersIds(ctx context.Context, dto *domain.UsersGetPercentDTO) (*domain.UsersIds, error)
...
// GetPersentOfUsers
// @Summary get persent of users
// @Tags user
// @Description return persent of random users
// @ID get-persent-users
// @Accept json
// @Produce json
// @Param input body domain.UsersGetPercentDTO true "persent of users"
// @Success 200 {object} domain.UsersIds
// @Failure 400 {object} badResponse
// @Router /user/get/persent [post]
func (h userHandler) GetPersentOfUsersIds(c *gin.Context) {
	...
	result, err := h.service.GetPersentOfUsersIds(ctx, request)
	if err != nil {
		c.JSON(http.StatusInternalServerError, badResponse{Message: err.Error()})
		return
	}
